(function () {
    ...
    /* ── 1. API base — auto-detect from widget.js src ────── */
    ...
    let WS = { ...DEFAULT_SETTINGS };       // Widget Settings (live)
    let postChatConfig = { enabled: false, fields: [] };
    let chatHasMessages = false;             // track if visitor ever sent a msg
    ...
    function trackPageStart() {
        ...
        fetch(API_BASE + '/visitor/page-start', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ visitor_id: visitorId, url: currentUrl, title: document.title })
        }).catch(() => {});
    }

    function patchHistory() {
        if (window.__cdPatched) return;
        window.__cdPatched = true;
        const _push    = history.pushState.bind(history);
        const _replace = history.replaceState.bind(history);
        history.pushState = function (...args) { _push(...args); setTimeout(trackPageStart, 0); };
        history.replaceState = function (...args) { _replace(...args); setTimeout(trackPageStart, 0); };
        window.addEventListener('popstate', () => setTimeout(trackPageStart, 0));
    }
    ...
    document.addEventListener('visibilitychange', () => {
        ...
        if (document.visibilityState === 'hidden') {
            const timeSpent = Math.floor((Date.now() - pageStartTime) / 1000);
            if (timeSpent >= 2 && lastTrackedUrl) {
                const pageData = JSON.stringify({
                    visitor_id: visitorId, url: lastTrackedUrl,
                    title: document.title, time_spent: timeSpent
                });
                navigator.sendBeacon(API_BASE + '/visitor/page', new Blob([pageData], { type: 'application/json' }));
            }
            sendLeave();
        } else if (document.visibilityState === 'visible') {
            // ★ Came back to the page — restore status to browsing/chatting
            trackVisitor();
            trackPageStart();
            // If chat is open, set status to chatting
            if (isOpen && view === 'chat') {
...
#cd-bubble.pos-left { right:auto; left:20px; }
#cd-badge {
...
#cd-popup.pos-left { right:auto; left:20px; }
...
#cd-branding {
    ...
    color:var(--muted); background:var(--popup-bg);
...
@media(max-width:480px){
    ...
    #cd-popup.pos-left { left:0; }
}
/* ── Close-chat confirmation popup ── */
#cd-confirm-overlay {
...
#cd-confirm-box {
    background:var(--bot-bg); border-radius:14px;
...
.cd-confirm-ok-btn:hover { background:#dc2626; }
...
.post-form-field  .answer {
  ...
  font-weight: 600;
    ...
    /* ── 7. State ────────────────────────────────────────── */
    let isOpen      = false;
    let view        = 'home'; // home | chat | postchat
    let unread      = 0;
    let filesToSend = [];
    let fbReady     = false;
    ...
    /* ── 8. Element refs ─────────────────────────────────── */
    let popup, badge, homeScreen, convScreen, footer, loadingEl;
    ...
    function applyWidgetSettings() {
        ...
        // Position
        const bubble_ = document.getElementById('cd-bubble');
        const popup_  = document.getElementById('cd-popup');
        if (bubble_ && popup_) {
            ...
            [bubble_, popup_].forEach(el => {
                el.style[side]  = sidePx + 'px';
                el.style[other] = 'auto';
            });
            bubble_.style.bottom = botPx + 'px';
            popup_.style.bottom  = (botPx + 70) + 'px';

            bubble_.classList.toggle('pos-left', WS.position === 'left');
            popup_.classList.toggle('pos-left', WS.position === 'left');
        }
    ...
    function togglePopup() {
        ...
        if (isOpen) {
            ...
            closeAllMenus();
            // ★ Check if already filled before showing on next open
            const alreadyFilled = localStorage.getItem('lc_postchat_filled_' + visitorId);
            if (!alreadyFilled && chatHasMessages && postChatConfig.enabled && !postChatShown) {
                view = 'postchat';
            }
        }
    }
    ...
    function renderMessage(msgId, d, isMine) {
        ...
        if (d.type === 'info_request' || d.type === 'info_filled' || d.type === 'system' || d.type === 'post_chat_response') {
            ...
            // ★ Render Post-Chat Responses beautifully
        ...
        if (d.fileUrl && d.fileType === 'image') {
            ...
            // ★ Fix old /storage/ URLs → /api/chat-files/
            ...
            // Legacy base64 image (backward compatible)
        ...
        if (chatImg) {
            ...
            chatImg.addEventListener('click', () => {
                document.getElementById('cd-modal-img').src = imgSrc;
                imgModal.classList.remove('cd-hidden');
            });
        }
        messagesBox.appendChild(w);
        messagesBox.scrollTop = messagesBox.scrollHeight;
    ...
    function renderPostChatForm() {
        ...
        fields.forEach((field, idx) => {
            ...
            if (field.type === 'thankyou') {
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.text || 'Thank you for chatting with us!')}</div>
                `;
            } else if (field.type === 'question') {
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.label || '')}</div>
                    <input class="cd-pc-input" type="text" data-field-idx="${idx}" placeholder="Your answer...">
                `;
            } else if (field.type === 'message') {
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.label || '')}</div>
                    <textarea class="cd-pc-textarea" data-field-idx="${idx}" placeholder="Write your message..." rows="3"></textarea>
                `;
            } else if (field.type === 'choice') {
                const opts = (field.options || ['Yes', 'No']).map((o, oi) =>
                    `<label class="cd-pc-radio-label">
                        <input type="radio" name="pc_field_${idx}" value="${esc(o)}" data-field-idx="${idx}">
                        ${esc(o)}
                    </label>`
                ).join('');
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.label || '')}</div>
                    <div class="cd-pc-radio-group">${opts}</div>
                `;
            } else if (field.type === 'dropdown') {
                const opts = (field.options || []).map(o => `<option>${esc(o)}</option>`).join('');
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.label || '')}</div>
                    <select class="cd-pc-select" data-field-idx="${idx}"><option value="">Select...</option>${opts}</select>
                `;
            } else if (field.type === 'multiple') {
                const opts = (field.options || []).map((o, oi) =>
                    `<label class="cd-pc-check-label">
                        <input type="checkbox" name="pc_field_${idx}" value="${esc(o)}" data-field-idx="${idx}">
                        ${esc(o)}
                    </label>`
                ).join('');
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.label || '')}</div>
                    <div class="cd-pc-check-group">${opts}</div>
                `;
            } else if (field.type === 'rating') {
                wrap.innerHTML = `
                    <div class="cd-pc-label">${esc(field.label || 'How would you rate this chat?')}</div>
                    <div class="cd-pc-rating-group">
                        <button type="button" class="cd-pc-rate-btn" data-value="up" data-field-idx="${idx}">👍</button>
                        <button type="button" class="cd-pc-rate-btn" data-value="down" data-field-idx="${idx}">👎</button>
                    </div>
                `;
            }
        ...
        function submitPostChatForm() {
            ...
            // ★ PUSH to Firebase so it shows in chat history for both sides
            ...
            postChatEl.innerHTML = `
                <div class="cd-pc-thankyou">
                    ...
                    <button class="cd-start-btn" style="margin-top:20px;" onclick="document.getElementById('cd-postchat').classList.remove('active'); document.getElementById('cd-home').classList.remove('cd-hidden');">
                        Back to Home
                    </button>
                </div>
            `;
    ...
    function bindUIEvents() {
        ...
        chatInput.addEventListener('input', function() {
            // Disable transition so height change is instant (no animation interference)
            ...
            // Re-enable transition after browser paints the new height
        ...
        document.getElementById('cd-modal-close').addEventListener('click', () => {
            imgModal.classList.add('cd-hidden');
        });

        document.getElementById('cd-transcript-btn').addEventListener('click', () => {
            closeAllMenus();
            appendBotMsg('Transcript sent to your email!');
        });
        ...
        document.getElementById('cd-close-chat-btn').addEventListener('click', () => {
            closeAllMenus();
            if (!chatHasMessages) {
                // No messages yet, just close the popup
                isOpen = false;
        ...
        document.getElementById('cd-confirm-ok').addEventListener('click', () => {
            ...
            // Set status to "ended" in Firebase so agent sees it too
            if (fbReady && activeRoomPath) {
                try {
                    if (window._cdSetStatus) window._cdSetStatus('ended');
                } catch(e) {}
            }
            ...
            // Show post-chat form if enabled and visitor had messages
            if (!alreadyFilled && chatHasMessages && postChatConfig.enabled && !postChatShown) {
                showView('postchat');
            } else {
                // No post-chat form — just go home and close
                showView('home');
                isOpen = false;
                popup.classList.remove('open');
            }
    ...
    Promise.all([
        ...
        // ★ Fetch post-chat form config
        fetch(API_BASE + '/widget/post-chat-config', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
            body:    JSON.stringify({ site_id: SITE_ID })
        }).then(r => r.json()).catch(() => ({ enabled: false, fields: [] })),
    ])
    .then(([config, banData, widgetSettingsData, postChatData]) => {
        ...
        // ★ Mark as authorized — enables tracking, heartbeat, etc.
        authorized = true;
    
        console.log('[ChatDesk360] Authorized');
        ...
        if (postChatData && postChatData.enabled) {
            postChatConfig = {
                enabled: true,
                fields:  postChatData.fields || []
            };
        }
    
        if (banData.banned) {
            console.warn('[ChatDesk360] Visitor is banned.');
            startUnbanPoll();
            return;
        }
    ...
    .catch(() => {
        // ★ Silent — domain not trusted or invalid site_id
        // No CSS, no HTML, no tracking injected
    });
    ...
    function hideWidget() {
        ...
        isOpen = false;
        clearInterval(heartbeatTimer);   // ★ already there, keep it
    }
    ...
    function initWidget(config) {
        // ★ INJECT CSS only after authorization (not for blocked domains)
        ...
        // ★ Start heartbeat only after authorization
        heartbeatTimer = setInterval(sendHeartbeat, 30000);
        ...
        // ★ Apply widget settings from database
        applyWidgetSettings();
    ...
    function initFirebase(fbConfig) {
        ...
        Promise.all([
            ...
            onValue(ref(db, `${basePath}/status`), snap => {
                ...
                if (status === 'ended') {
                    ...
                    if (!alreadyFilled && chatHasMessages && postChatConfig.enabled && !postChatShown) {
                        ...
                        showView('home');
                        // Optionally close popup after a moment
                        if (!isOpen) return;
                        setTimeout(() => {
                            isOpen = false;
                            popup.classList.remove('open');
            ...
            fbReady = true;
            /* ── Bridge: allow UI to set chat status ── */
            window._cdSetStatus = function(status) {
                if (!fbReady || !activeRoomPath) return;
                set(ref(db, `${activeRoomPath}/status`), status);
            };
            
            /* ── Bridge: push post-chat response to chat messages ── */
